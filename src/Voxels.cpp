...
Vector4f VoxelArray::idx2location(const unsigned idx) const {
    ...
    */
    ...
    return Vector4f{xyz[0] * _grid_len + _origin(0) + _grid_len / 2, 
                    xyz[1] * _grid_len + _origin(1) + _grid_len / 2, 
                    xyz[2] * _grid_len + _origin(2) + _grid_len / 2, 
                    1};
}
...
Vector4f VoxelArray::xyz2location(const unsigned x, const unsigned y, const unsigned z) const {
    ...
    return Vector4f{x * _grid_len + _origin(0) + _grid_len / 2, 
                    y * _grid_len + _origin(1) + _grid_len / 2, 
                    z * _grid_len + _origin(2) + _grid_len / 2, 
                    1};
}
